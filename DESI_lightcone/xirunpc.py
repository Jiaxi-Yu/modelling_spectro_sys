...
def compute_correlation_function(corr_type, edges, distance, nthreads=8, gpu=False, dtype='f8', wang=None, split_randoms_above=30., weight_type='default', tracer='ELG', tracer2=None, recon_dir=None, rec_type=None, njack=120, option=None, mpicomm=None, mpiroot=None, cat_read=None, dat_cat=None, ran_cat=None, rpcut=None, thetacut=None, catas_type=None, **kwargs):
    ...
    if mpicomm is None or mpicomm.rank == mpiroot:
        ...
        data, randoms = io.read_clustering_positions_weights(distance, name=['data', 'randoms'], recon_dir=recon_dir,rec_type=rec_type, tracer=tracer, option=option, cat_read=cat_read, dat_cat=dat_cat, ran_cat=ran_cat, catas_type=args.catas_type, **catalog_kwargs)

        if (with_shifted) & (cat_read == None):
            shifted = randoms  # above returned shifted randoms
            randoms = io.read_clustering_positions_weights(distance, name='randoms', rec_type=False, tracer=tracer, option=option, catas_type=args.catas_type, **catalog_kwargs)
        ...
        if not autocorr:
            data, randoms = io.read_clustering_positions_weights(distance, name=['data', 'randoms'], rec_type=rec_type, tracer=tracer2, option=option, catas_type=args.catas_type, **catalog_kwargs)
            if with_shifted:
                shifted = randoms
                randoms = io.read_clustering_positions_weights(distance, name='randoms', rec_type=False, tracer=tracer2, option=option, catas_type=args.catas_type, **catalog_kwargs)
...
def corr_fn(file_type='npy', region='', tracer='ELG', tracer2=None, zmin=0, zmax=np.inf, recon_dir='n',rec_type=False, weight_type='default', bin_type='lin', njack=0, nrandoms=8, split_randoms_above=10, out_dir='.', option=None, wang=None, rpcut=None, thetacut=None, catas_type=None):
