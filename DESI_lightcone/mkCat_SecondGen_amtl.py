...
parser.add_argument("--catas", help="if y, apply catastrophics)", default = 'y')
...
if args.mkclusdat == 'y':
    ...
    ct.mkclusdat(os.path.join(readdir, args.tracer + notqso), tp = args.tracer, dchi2 = None, tsnrcut = 0, zmin = zmin, zmax = zmax, use_map_veto = args.use_map_veto, subfrac=subfrac,zsplit=zsplit, ismock=True, ccut=args.ccut, iscatas=catas)#,ntilecut=ntile,ccut=ccut)
...
rcols=['Z','WEIGHT','WEIGHT_SYS','WEIGHT_COMP','WEIGHT_ZFAIL','TARGETID_DATA','Z_realistic','Z_failures']
if args.mkclusran == 'y':
