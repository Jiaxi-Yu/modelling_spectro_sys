...
def get_clustering_positions_weights(catalog, distance, zlim=(0., np.inf),maglim=None, weight_type='default', name='data', return_mask=False, option=None, catas_type=None, **kwargs):
    #############  catas_type ####################
    P0       = 4000
    if catas_type is not None:
        ...
        print('shifted to catastrophics Z and WEIGHT_FKP')
...
def read_clustering_positions_weights(distance, zlim =(0., np.inf), maglim=None, weight_type='default', name='data', concatenate=False, option=None, region=None, cat_read=None, dat_cat=None, ran_cat=None, catas_type=None, **kwargs):
    #print(kwargs)
    ...
    if cat_read == None:
        def read_positions_weights(name):
            ...
            for reg in region:
                ...
                if isscalar:
                    cat_fns = [cat_fns]
                positions_weights = [get_clustering_positions_weights(Table.read(cat_fn), distance, zlim=zlim, maglim=maglim, weight_type=weight_type, name=name, option=option, catas_type=catas_type, **kwargs) for cat_fn in cat_fns]
    ...
    if cat_read != None:
        def read_positions_weights(name):
            ...
            for reg in region:
                ...
                positions_weights = [get_clustering_positions_weights(cat_read, distance, zlim=zlim, maglim=maglim, weight_type=weight_type, name=name, option=option, catas_type=catas_type, **kwargs)]
                if name == 'data':
                    positions.append(positions_weights[0][0])
                    weights.append(positions_weights[0][1])
