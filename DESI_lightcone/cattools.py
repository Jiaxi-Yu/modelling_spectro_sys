...
def mkclusdat(fl,weighttileloc=True,zmask=False,tp='',dchi2=9,tsnrcut=80,rcut=None,ntilecut=0,ccut=None,ebits=None,zmin=0,zmax=6,write_cat='y',splitNS='n',return_cat='n',compmd='ran',kemd='',wsyscol=None,use_map_veto='',subfrac=1,zsplit=None,ismock=False,iscatas=False):
    import LSS.common_tools as common
    ...
    if iscatas:
        print('adding catastrophics on the fly')
        # implement the catastrophical failure
        dvcatas   = 1000
        dvcatasmax= 10**5.65
        ## initialise the columns
        ### realistic catastrophics
        frac_tot  = 0.27
        frac_nores= 0.751
        Nreal     = int(len(ff)*frac_tot/100)
        Nrealnores= int(Nreal*frac_nores) if int(Nreal*frac_nores)%2==0 else int(Nreal*frac_nores)-1
        ff['Z_realistic']= ff['Z'].copy()

        # frac_tot% of catastrophics rate
        inds      = np.array(random.sample(range(0, len(ff)), Nreal))
        random.shuffle(inds) # we need this because AbacusSummit "Z" was sorted
        ## frac_nores*100% ordinary zfailures due to various reasons
        def inv_trans(uni):
            return 3321/500-np.exp((erfinv(1-200/99*uni)*243*np.sqrt(2)+657)/1000)
            #return 6.499-np.exp((253*erfinv(1-2000/991*uniform)+37*2**3.5)/125/2**2.5)            
        def func_validate(uniform):
            dv_pos    = inv_trans(uniform)
            rest      = (dv_pos<np.log10(dvcatas))|(dv_pos>np.log10(dvcatasmax))|(~np.isfinite(dv_pos))
            while np.sum(rest)>0:
                dv_pos[rest] = inv_trans(np.random.rand(np.sum(rest))) 
                rest  = (dv_pos<np.log10(dvcatas))|(dv_pos>np.log10(dvcatasmax))|(~np.isfinite(dv_pos))            
            return dv_pos
        exponent  = func_validate(np.random.rand(int(Nrealnores/2)))
        dv        = np.append(10**exponent,-10**exponent)
        random.shuffle(dv)
        ff['Z_realistic'][inds[:Nrealnores]] += dv/299792*(1+ff['Z'][inds[:Nrealnores]])
        ## (1-frac_nores)*100% extra zfailures due to z=1.32 sky residuals
        ff['Z_realistic'][inds[Nrealnores:]] \
                  = np.random.normal(1.32,0.006, size=Nreal-Nrealnores)
        
        # 1% catastrophics rate
        ## hypothetical 1% catatrophics
        Nfail     = int(len(ff)*1/100) if int(len(ff)*1/100)%2==0 else int(len(ff)*1/100)+1
        ff['Z_failures'] = ff['Z'].copy()
        inds      = random.sample(range(0, len(ff)), Nfail)
        exponent  = func_validate(np.random.rand(int(Nfail/2)))
        dv        = np.append(10**exponent,-10**exponent)
        random.shuffle(dv)
        ff['Z_failures'][inds] += dv/299792*(1+ff['Z'][inds])    
        print('implement redshift selection for real redshift and catastroohically contaminated redshift')
        selz = ((ff['Z'] > zmin)|(ff['Z_realistic'] > zmin)|(ff['Z_failures'] > zmin))
...
#    ff['WEIGHT'] *= ff['WEIGHT_SYS']
    ...
    kl = ['RA','DEC','Z','WEIGHT','TARGETID','NTILE','WEIGHT_SYS','WEIGHT_COMP','WEIGHT_ZFAIL','PHOTSYS','FRAC_TLOBS_TILES','Z_realistic','Z_failures']#,'WEIGHT_FKP']
