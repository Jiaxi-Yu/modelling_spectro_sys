...
def parse_args():
    ...
    parser.add_argument("--Om", help="the Omega_m of the galaxy mock", type=float, default=None, required=True)
...
def inv_trans(uniform, func='G', sigma=None):
    if func == 'G': # Gaussian profile
        invfunc = sigma*sqrt(-2*log(uniform))*cos(2*pi*uniform)
    elif func == 'L': # Lorentzian profile
        invfunc = tan((uniform-1/2)*pi)*sigma
    elif func == 'lnG':
        invfunc = 3321/500-np.exp((erfinv(1-200/99*uni)*243*np.sqrt(2)+657)/1000)
    return invfunc   
...
def spec_systematics(uniform, func='L', sigma=None, maxdv=None, dvcatas=1000, dvcatasmax=10**5.65):
    ...
    if   func == 'L':
        ...
        while np.sum(outliers)>0:
            Delta_v[outliers] =  inv_trans(np.random.rand(np.sum(outliers))) 
            outliers = abs(Delta_v)>maxdv
    elif func == 'lnG':
        dv_tot       = Delta_v.copy()
        outliers     = (dv_tot<np.log10(dvcatas))|(dv_tot>np.log10(dvcatasmax))|(~np.isfinite(dv_tot))
        while np.sum(outliers)>0:
            dv_tot[outliers] = inv_trans(np.random.rand(np.sum(outliers))) 
            outliers = (dv_tot<np.log10(dvcatas))|(dv_tot>np.log10(dvcatasmax))|(~np.isfinite(dv_tot))  
...
if geometry == 'box':
    ...
    np.savetxt(output,dv,header='Delta-v, add it to the peculiar velocity of your galaxy mocks')
else:
